...
import day49_inheritance.BookShelf.Book;
...
public class Shelf {
    ...
    public static boolean isIDUnique(int id){
         //Todo Buraya odaklan burada hata var.
        for (int i = 0; i < theBooks.size(); i++) {
            if(theBooks.get(0).getId()==theBooks.get(i).getId()){
                return false;
            }
    ...
    public static Book findBookByPartialTitle(String title) {
        //Todo Burada da sorun var
        for (int i = 0; i < theBooks.size(); i++) {
             if(theBooks.get(i).getTitle().toLowerCase().contains(title.toLowerCase().substring(0,5))){
     ...
     * if there is no match, no action needed
     */
